...
const JobApplicationForm = ({ jobData }) => {
  ...
  const validateField = (field, value) => {
    ...
    if (field.isRequired) {
      ...
      } else if (!value || (typeof value === 'string' && value.trim().length === 0) || (Array.isArray(value) && value.length === 0)) {
        return `${field.fieldLabel} is required`;
      }
    }
        ...
        try {
          ...
          switch (fieldType) {
            case 'voice_recording':
              result = await dispatch(uploadVoiceRecording({ file, maxDuration: 300 })).unwrap();
              fileUploads[fieldName] = result.data;
              break;
              
            case 'video_recording':
              result = await dispatch(uploadVideoRecording({ file, quality: 'auto:low' })).unwrap();
              fileUploads[fieldName] = result.data;
              break;
            ...
            default:
              ...
              const uploadedUrl = Array.isArray(result.data) ? result.data[0] : result.data;
          ...
          throw new Error(`Failed to upload ${fieldName}: ${err.message || 'Upload failed'}`);
        }
      }
    }
          ...
          <div key={section._id || sectionIndex} className="bg-linear-to-br from-white via-gray-50 to-white rounded-2xl shadow-xl border border-gray-100 p-8 mb-8 transform transition-all duration-200 hover:shadow-2xl">
            ...
            <div className="space-y-6">
              <FormField
                ...
                onFileChange={(fieldName, file) => handleFileChange(fieldName, file, mediaType)}
            ...
            {section.fields
              ...
              .map((field, fieldIndex) => (
                <FormField
                  ...
                  value={formData[field.fieldName] || ''}
                  onChange={handleFieldChange}
                  onFileChange={(fieldName, file) => handleFileChange(fieldName, file, field.fieldType)}
          ...
          {(error || errors.submit) && (
            <div className="bg-linear-to-r from-red-50 to-red-100 border-2 border-red-300 rounded-2xl p-6 shadow-lg">
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-bold text-red-800 mb-1">Submission Error</h3>
                  <p className="text-sm text-red-700">
                    {error?.message || errors.submit || 'An error occurred. Please try again.'}
                  </p>
                </div>
              </div>
            </div>
